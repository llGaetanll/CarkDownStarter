function carkDown(text, p0 = 0, lvl = 0) {
  ...
  if (text === "---") return '<div class="divider"></div>';

  // if there is no formatting and it's our first time seeing the text
  if (!text.includes("[") && p0 == 0) return `<p>${text}</p>`;
  ...
  while (p0 < textChars.length) {
    ...
    if (textChars[p0] == "]" && opNum == clNum) {
      ...
      let p5 = p1;
      while (p5 < p2) {
        if (textChars[p5] == "." || textChars[p5] == "#") break;
        p5++;
      }
      ...
      classes.forEach((c, i) => {
        ctxt += c;
        if (i != classes.length - 1) ctxt += " ";
      });

      ids.forEach((id, i) => {
        itxt += id;
        if (i != ids.length - 1) itxt += " ";
      });

      let classText = ` class="${ctxt}"`,
        idText = ` id="${itxt}"`;
      ...
      switch (operation) {
        case "h1":
          text =
            text.substring(0, p1) +
            `<h1${classes.length > 0 ? classText : ""}${
              ids.length > 0 ? idText : ""
            }>${text.substring(p2 + 1, p3)}</h1>` +
        ...
        case "h2":
          text =
            text.substring(0, p1) +
            `<h2${classes.length > 0 ? classText : ""}${
              ids.length > 0 ? idText : ""
            }>${text.substring(p2 + 1, p3)}</h2>` +
            text.substring(p3 + 1, text.length);
        ...
        case "h3":
          text =
            text.substring(0, p1) +
            `<h3${classes.length > 0 ? classText : ""}${
              ids.length > 0 ? idText : ""
            }>${text.substring(p2 + 1, p3)}</h3>` +
            text.substring(p3 + 1, text.length);
        ...
        case "h4":
          text =
            text.substring(0, p1) +
            `<h4${classes.length > 0 ? classText : ""}${
              ids.length > 0 ? idText : ""
            }>${text.substring(p2 + 1, p3)}</h4>` +
            text.substring(p3 + 1, text.length);
        ...
        case "h5":
          text =
            text.substring(0, p1) +
            `<h5${classes.length > 0 ? classText : ""}${
              ids.length > 0 ? idText : ""
            }>${text.substring(p2 + 1, p3)}</h5>` +
            text.substring(p3 + 1, text.length);
        ...
        case "^":
          ...
          text =
            text.substring(0, p1) +
            `<img src="${text.substring(p2 + 1, p3)}"${
              classes.length > 0 ? classText : ""
            }${ids.length > 0 ? idText : ""}>` +
            text.substring(p3 + 1, text.length);
        ...
        case "*":
          ...
          text =
            text.substring(0, p1) +
            `<b${classes.length > 0 ? classText : ""}${
              ids.length > 0 ? idText : ""
            }>${text.substring(p2 + 1, p3)}</b>` +
            text.substring(p3 + 1, text.length);
        ...
        case "_":
          ...
          text =
            text.substring(0, p1) +
            `<i${classes.length > 0 ? classText : ""}${
              ids.length > 0 ? idText : ""
            }>${text.substring(p2 + 1, p3)}</i>` +
            text.substring(p3 + 1, text.length);
        ...
        case "$":
          ...
          const title =
            text.substring(p2 + 1, p4).indexOf("[") > 0
              ? carkDown(text.substring(p2 + 1, p4), 0, lvl)
              : text.substring(p2 + 1, p4);

          text =
            text.substring(0, p1) +
            `<a href="${text.substring(p4 + 1, p3).replace(/ /g, "")}"${
              classes.length > 0 ? classText : ""
            }${ids.length > 0 ? idText : ""}>${title}</a>` +
            text.substring(p3 + 1, text.length);
          break;
        case "YT":
          // youtube embeds
      ...
      if (textChars.length > p3 - p0)
        // <p> only when other text comes before or after it
